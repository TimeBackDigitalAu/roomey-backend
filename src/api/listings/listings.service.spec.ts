...
describe('ListingsService', () => {
  ...
  describe('searchListings', () => {
    ...
    it('should throw BadRequestException for validation errors', async () => {
      ...
      jest
        .spyOn(service as unknown as { validateInput: jest.Mock }, 'validateInput')
        .mockImplementation(() => {
          throw new ZodError([
            {
              code: 'invalid_type',
    ...
    it('should handle database errors gracefully', async () => {
      ...
      await expect(service.searchListings(searchDto)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('getListingById', () => {
    it('should return listing when valid UUID is provided', async () => {
      ...
      expect(mockPrismaClient.$queryRaw).toHaveBeenCalledWith(expect.stringContaining('SELECT'));
    });

    it('should throw BadRequestException for invalid UUID format', async () => {
      const invalidId = 'invalid-uuid';

      await expect(service.getListingById(invalidId)).rejects.toThrow(BadRequestException);
    ...
    it('should throw NotFoundException when listing does not exist', async () => {
      ...
      await expect(service.getListingById(validId)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('createListing', () => {
    it('should create listing successfully with valid data', async () => {
      ...
      mockPrismaClient.$transaction.mockImplementation(
        async (callback: (prisma: { $queryRaw: jest.Mock }) => Promise<unknown>) => {
          return callback({
            $queryRaw: jest
              .fn()
    ...
    it('should throw BadRequestException when no users are available', async () => {
      ...
      await expect(service.createListing(createDto)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException for validation errors', async () => {
      const invalidDto = { title: 'A', price: -100 }; // Invalid data
      ...
      await expect(
        service.createListing(invalidDto as unknown as Parameters<typeof service.createListing>[0])
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should handle database errors gracefully', async () => {
      ...
      await expect(service.createListing(createDto)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('updateListing', () => {
    it('should update listing successfully with valid data', async () => {
      ...
      jest.spyOn(service, 'getListingById').mockResolvedValue(mockUpdatedListing);
    ...
    it('should throw NotFoundException when listing does not exist', async () => {
      ...
      await expect(service.updateListing(validId, updateDto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException for validation errors', async () => {
      ...
      await expect(service.updateListing(validId, invalidDto)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('deleteListing', () => {
    it('should delete listing successfully with valid UUID', async () => {
      ...
      await service.deleteListing(validId).then((result: { message: string; deleted_at: Date }) => {
    ...
    it('should throw BadRequestException for invalid UUID format', async () => {
      ...
      await expect(service.deleteListing(invalidId)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw NotFoundException when listing does not exist', async () => {
      ...
      await expect(service.deleteListing(validId)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findNearby', () => {
    ...
    it('should throw BadRequestException for invalid coordinates', async () => {
      ...
      await expect(service.findNearby(invalidLat, lon)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should handle database errors gracefully', async () => {
      ...
      await expect(service.findNearby(lat, lon)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('healthCheck', () => {
    it('should return healthy status when database is accessible', async () => {
      ...
      await service
        .healthCheck()
        .then((result: { status: string; timestamp: Date; service: string }) => {
  ...
  describe('convertBigInts', () => {
    it('should convert BigInt values to numbers', () => {
      ...
      expect((result as { nested: { array: number[] } }).nested.array).toEqual([1, 2, 3]);
    });
    ...
    it('should handle null and undefined values', () => {
      ...
      expect((result as { description: undefined }).description).toBeUndefined();
    });
  ...
  describe('validateInput', () => {
    it('should validate input successfully with valid schema', () => {
      ...
      const result = (
        service as unknown as { validateInput: (schema: unknown, data: unknown) => unknown }
