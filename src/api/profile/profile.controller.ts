...
export class ProfileController {
  ...
  async searchProfiles(@Query('q') searchTerm: string, @Request() req: any) {
    ...
    return this.profileService.searchProfiles(searchTerm, req.user.userId, req.user.role);
  }
  ...
  async getMyProfile(@Request() req: any) {
    return this.profileService.getProfile(req.user.userId, req.user.userId, req.user.role);
  }
  ...
  @ApiResponse({ status: 404, description: 'Profile not found' })
  async getProfile(@Param('userId', ParseUUIDPipe) userId: string, @Request() req: any) {
    return this.profileService.getProfile(userId, req.user.userId, req.user.role);
  }
  ...
  async updateProfile(
    ...
    return this.profileService.updateProfile(userId, profileData, req.user.userId, req.user.role);
  }
  ...
  async deleteMyProfile(@Request() req: any) {
    return this.profileService.deleteProfile(req.user.userId, req.user.userId, req.user.role);
  }
  ...
  @ApiResponse({ status: 403, description: 'Access denied' })
  @ApiResponse({ status: 404, description: 'Profile not found' })
  async deleteProfile(@Param('userId', ParseUUIDPipe) userId: string, @Request() req: any) {
    return this.profileService.deleteProfile(userId, req.user.userId, req.user.role);
  }
}
